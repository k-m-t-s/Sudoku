...
FPS = 30
window = pg.display.set_mode((570,470))
xcounter = -1
ycounter = 0
board = []
lists2 =[]
            ...
            for a in horizontal:
                ...
                for b in horizontal:
                    if a == b:
                        counter += 1
                if counter != 1:
                    print("ERROR - double in y "+ str(y))
                    print(counter)
                if counter == 1:
                    if y != 8:
                        continue
                    if y == 8:
                        print("Y axis is cleared.")
        
        for x in range(9):
            for a in lists2[1][0][x]:
                vertical.append(a)
            for a in vertical:
                counter = 0
                for b in vertical:
                    if a == b:
                        counter += 1
                if counter != 1:
                    print("ERROR - double in x "+ str(x))
                if counter == 1:
                    if x != 8:
                        continue
                    if x == 8:
                        print("X axis is cleared.")
        
        for z in range(1,10):
            algo.block(z)
            for y in range(3):
                for x in range(3):
                    for a in lists2[1][fy+y][fx+x]:
                        block.append(a)
            for a in board:
                counter = 0
                for b in board:
                    if a == b:
                        counter += 1
                if counter != 1:
                    print("ERROR - double in box. fx "+ str(fx)+ " fy "+ str(fy))
                if counter == 1:
                    if z != 9:
                        continue
                    if z == 9:
...
class submit(box):
    def checkstate(self, event):
        ...
        if pg.MOUSEBUTTONDOWN == event.type:
            if self.recta.collidepoint(x,y):
                return True
        return False
...
class algorithm:
    ...
    def checkremove(self):
        ...
        print("one single other duplicates ")
        for y in range(9):
            print(lists2[1][y])
            if y == 2 or y ==5:
                print(" ")
...
def main():
    ...
    while run:
        ...
        for rectan in highlight_boxes:
            rectan.draw(window)
        if submitbutton.checkstate(event):
            solve = True
        if solve == True:
